...
export const CardContent = ({ trackScroll }: { trackScroll: boolean }) => {
	...
	return (
		<>
			<m.div
				className={`overflow-hidden w-[600px] h-full flex transition-colors rounded-[4rem]  flex-col p-12 col-span-2 row-span-12 min-h-0 duration-500 ${
				...
				{!info ? (
					...
					<div className="flex-col flex flex-1 text-background">
						...
						<div className="flex-1 items-center">
							<m.p
								className="text-4xl mt-4 text-wrap leading-[3.5rem]"
							...
							<m.p
								className="text-4xl mt-10 text-pretty"
								initial={{ opacity: 0, y: 50 }}
								animate={{ opacity: 100, y: 0 }}
								transition={{ delay: 1 }}
							>
							...
							>
								Currently working with <Bold>Next</Bold>, <Bold>React</Bold> and{" "}
								<Bold>Express</Bold>
							</m.p>
							<m.p
								className="text-4xl mt-10 text-pretty"
							...
							>
								...
								<Bold>challenging</Bold> project that could teach me new things
								and make me a <Bold>better dev</Bold>
							</m.p>
						</div>
					</div>
				)}
